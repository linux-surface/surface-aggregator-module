 ...
 *
 * Returns zero on success or if the frame is incomplete, -ENOMSG if the start
 * of the message is invalid, -EBADMSG if any (frame-header or payload) CRC is
 * ivnalid, or -EMSGSIZE if the SSH message is bigger than the maximum message
 * length specified in the @maxlen parameter.
 *
